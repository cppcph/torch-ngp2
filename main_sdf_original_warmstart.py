...
if __name__ == '__main__':
    ...
    original_mesh.export("SMPLTemplateNormalized6.obj")

    train_dataset0=SDFDatasetNoNormalization("SMPLTemplateNormalized6.obj", size=100, num_samples=2**14)
    train_loader0 = torch.utils.data.DataLoader(train_dataset0, batch_size=1, shuffle=True)
    valid_dataset0 = SDFDatasetNoNormalization("SMPLTemplateNormalized6.obj", size=1, num_samples=2**14) # just a dummy
    ...
    name="SMPL/"+"TrainReference"+"lr_"+str(opt.lr)+"_"+time_stamp+opt.path.split("/")[-1]
    ...
    valid_dataset1= SDFDatasetNoNormalization("SMPLOneDefNormalized.obj", size=1, num_samples=2**14) # just a dummy
