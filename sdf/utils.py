...
class TrainerOurs(object):
    def __init__(self, 
        ...
        self.console = Console()
        self.mesh = mesh
        self.number_random_sample_every_step = 2**15
        model.to(self.device)
        if self.world_size > 1:
            model = torch.nn.SyncBatchNorm.convert_sync_batchnorm(model)
            model = torch.nn.parallel.DistributedDataParallel(model, device_ids=[local_rank])
    ...
    def train_step(self, data):
        ...
        return pred, y, 100*loss_sd+ 10*loss_grad + eikonal_l1_avg+100*loss_sd_grid+10*loss_grad_grid,100*loss_sd,10*loss_grad,eikonal_l1_avg,100*loss_sd_grid,10*loss_grad_grid,loss_sd_ref_grid,loss_grad_ref_grid,loss_sd_ref_surface,loss_grad_ref_surface
