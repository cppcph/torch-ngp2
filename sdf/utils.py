...
class Trainer(object):
    ...
    def evaluate_one_epoch(self, loader):
        ...
        total_loss = 0
        total_loss_sd_grid_val =0
        total_loss_sd_surface_val =0
        if self.local_rank == 0:
            for metric in self.metrics:
                metric.clear()
        ...
        with torch.no_grad():
            ...
            for data in loader:    
                ...
                with torch.cuda.amp.autocast(enabled=self.fp16):
                    preds, truths, loss, loss_sd_ref_grid,loss_grad_ref_grid, loss_sd_ref_surface,loss_grad_ref_surface= self.eval_step(data)
                ...
                total_loss_sd_grid_val += loss_sd_ref_grid_val
                total_loss_sd_surface_val += loss_sd_ref_surface_val
        ...
        average_loss = total_loss / self.local_step
        average_loss_sd_grid_val= total_loss_sd_grid_val / self.local_step
        average_loss_sd_surface_val= total_loss_sd_surface_val / self.local_step
...
class TrainerWithFixedSubspace(object):
    def __init__(self, 
                 name, # name of this experiment
                 model, # network 
                 criterion=None, # loss function, if None, assume inline implementation in train_step
                 ...
                 use_tensorboardX=True, # whether to use tensorboard for logging
                 scheduler_update_every_step=False, # whether to call scheduler.step() after every train step
                 subspace=np.array([[1,1,1,1,1,1,1]])/7
                 ):
    ...
    def train_step(self, data):
        ...
        subspace_rep= subspace.repeat(X.shape[0],1)
        pred,_ = self.model(X,subspace_rep)
        loss = self.criterion(pred, y)

        if self.use_tensorboardX:
            subspace_rep= subspace.repeat(self.reference_grid_points.shape[0],1)
            ...
            subspace_rep= subspace.repeat(self.reference_surface_points.shape[0],1)
...
class TrainerSubspace(object):
    ...
    def train_step(self, data):
        # assert batch_size == 1
        X = data["points"][0] # [B, 3]
        X0= data["reference_points_on_original_mesh"][0] # [B, 3]
        flag= data["surface_flag"][0]
        subspaces= data["subspace"][0] # [B, 4]
        y = data["sdfs"][0] # [B]
        
        pred_sdf, pred_warping = self.model(X,subspaces)
        loss_sd= self.criterion(pred_sdf, y)
...
class TrainerTestPreencoder(object):
    def __init__(self, 
                 name, # name of this experiment
                 model, # network 
                 criterion=None, # loss function, if None, assume inline implementation in train_step
